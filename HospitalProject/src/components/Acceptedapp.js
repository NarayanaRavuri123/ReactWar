...
function Acceptedapp() {
    const [category,setcategory]=useState([]);
    const initialValues = { Department: ""};
    const [formValues, setFormValues] = useState(initialValues);
        const getcategory = async (FullName) =>{
            const res= await fetch('https://localhost:7081/api/Provider/AcceptedAppointments?OurDoctor='+FullName+'');
...
<center>
    ...
    <tbody>
    {
        ...
        <td><Link to = "/Prescription" class="appointment-btn scrollto">Generate Report</Link><br/><br/>
        </td>
        </tr>
