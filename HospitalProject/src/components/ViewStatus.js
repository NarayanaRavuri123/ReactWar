...
function ViewStatus() {
    ...
    const [formValues, setFormValues] = useState(initialValues);
        const getcategory = async (AppointmentId) =>{
            const res= await fetch('https://localhost:7081/api/Patient/ViewStatus?AppointmentId='+AppointmentId+'');
    ...
    const handleSubmit = (e) => {
        e.preventDefault();
        localStorage.setItem("AppointmentId",formValues.AppointmentId)
    };

    const CancelAppointment = (AppointmentId) =>{
        alert(AppointmentId);
        axios.put('https://localhost:7081/api/Patient/CancelAppointment?AppointmentId='+AppointmentId+'')
        .then((result)=>{
          alert("Rejected");
  ...
  return (
    ...
    <div>
      <center>
        ...
        />
        <br/><br/>
        <center><button className={styles.formSubmitBtn} id="contact_button" onClick={()=>getcategory(formValues.AppointmentId)}>View</button></center>
</form>
...
<center>
    ...
    <tbody>
    {
        ...
        <td><Link class="appointment-btn scrollto" to="/ViewPrescription">View Report </Link></td>
