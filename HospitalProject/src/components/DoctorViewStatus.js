...
function DoctorViewStatus() {
    const [category,setcategory]=useState([]);
    const initialValues = { Department: ""};
    const [formValues, setFormValues] = useState(initialValues);
        const getcategory = async (FullName) =>{
            const res= await fetch('https://localhost:7081/api/Provider/ViewStatus?OurDoctor='+FullName+'');
            const getdata =await res.json();
            setcategory(getdata);
        }
        getcategory(localStorage.getItem("FullName"));
...
<center>
    ...
    <tbody>
    {
        ...
        <td><button class="btn btn-success" onClick={()=>AcceptAppointment(getcate.appointmentId)}>Accept</button><br/><br/>
        <button class="btn btn-danger" onClick={()=>RejectAppointment(getcate.appointmentId)}>Reject</button></td>
...
export default DoctorViewStatus;
